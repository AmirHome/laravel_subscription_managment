...
<div class="card">
    <div class="card-header">
        {{ trans('laravel_subscription_managment::global.create') }} {{ trans('laravel_subscription_managment::cruds.subscription.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("ajax.subscriptions.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="subscriber">{{ trans('laravel_subscription_managment::cruds.subscription.fields.subscriber') }}</label>
                <input class="form-control {{ $errors->has('subscriber') ? 'is-invalid' : '' }}" type="number" name="subscriber" id="subscriber" value="{{ old('subscriber', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subscriber_type') ? 'is-invalid' : '' }}" type="text" name="subscriber_type" id="subscriber_type" value="{{ old('subscriber_type', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('unlimited') ? 'is-invalid' : '' }}" name="unlimited" id="unlimited">
                    ...
                    @foreach(\Amirhome\LaravelSubscriptionManagment\Models\Subscription::UNLIMITED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('unlimited', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('suppressed_at') ? 'is-invalid' : '' }}" type="text" name="suppressed_at" id="suppressed_at" value="{{ old('suppressed_at') }}">
