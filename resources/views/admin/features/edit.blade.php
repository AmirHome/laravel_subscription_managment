...
<div class="card">
    ...
    {{ trans('laravel_subscription_managment::global.edit') }} {{ trans('laravel_subscription_managment::cruds.subscriptionFeature.title_singular') }}
    </div>

    <div class="card-body">
    <form method="POST" action="{{ route('ajax.subscription-features.update', [$subscriptionFeature->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('laravel_subscription_managment::cruds.subscriptionFeature.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $subscriptionFeature->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $subscriptionFeature->code) }}" required>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $subscriptionFeature->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('group') ? 'is-invalid' : '' }}" name="group_id" id="group_id">
                    @foreach($groups as $id => $entry)
                        <option value="{{ $id }}" {{ (old('group_id') ? old('group_id') : $subscriptionFeature->group->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('group'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('active') ? 'is-invalid' : '' }}" name="active" id="active">
                    <option value="" disabled {{ old('active', isset($subscriptionFeature) ? $subscriptionFeature->active : null) === null ? 'selected' : '' }}>{{ trans('laravel_subscription_managment::global.pleaseSelect') }}</option>
                    @foreach($activeSelect as $key => $label)
                        <option value="{{ $key }}" {{ (string) old('active', isset($subscriptionFeature) ? $subscriptionFeature->active : null) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('limited') ? 'is-invalid' : '' }}" name="limited" id="limited">
                    <option value="" disabled {{ old('limited', isset($subscriptionFeature) ? $subscriptionFeature->limited : null) === null ? 'selected' : '' }}>{{ trans('laravel_subscription_managment::global.pleaseSelect') }}</option>
                    @foreach($limitedSelect as $key => $label)
                        <option value="{{ $key }}" {{ (string) old('limited', isset($subscriptionFeature) ? $subscriptionFeature->limited : null) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
